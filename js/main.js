...
function endGame(win) {
  ...
  if (win) {
    h2.appendChild(document.createTextNode("You won"));
    stopTimer();
    let folts;
    if (rongTrays < 5) {
      p.appendChild(document.createTextNode(`Brilliantly Played, `));
      folts = `<span>${rongTrays}</span> fault`;
    } else if (rongTrays < 10) {
      p.appendChild(document.createTextNode(`Nice Game, `));
      folts = `<span>${rongTrays}</span> fault`;
    } else if (rongTrays < 15) {
      p.appendChild(document.createTextNode(`Good Game,  `));
      folts = `<span>${rongTrays}</span> fault`;
    ...
    h2.appendChild(document.createTextNode("Nice Try"));
    stopTimer();
  }
